...
export default function Home () {
	...
	const player = async (sheet, mult = 1) => {
		if (isRunning) return;
		isRunning = true;
		const timer = function (ms) { return new Promise((resolve) => setTimeout(resolve, ms)); };
		const on = "#3BA067";
		const off = "rgba(255,255,255,0.2)";
		let i = 0;
		const innerSave = flasherRef.current.innerHTML;
		for (const dur of sheet) {
			const active = i % 2 === 0;
			if (active) {
		...
		<>
			...
			<main style={{ textAlign: "center" }}>
				...
				<div
					...
					onClick={() => player(sheets.jingle, 2)}
				>
					<span>Run</span>
				</div>
			</main>
